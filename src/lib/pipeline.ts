...
const KEY_SOURCE = "(?:vless|vmess|trojan|ss|ssr|hy2|hysteria2?|tuic|wireguard|naive\\+?[a-z]*)://\\S+";

/** Всегда возвращает свежий regex — глобальные флаги stateful и ломают .test(). */
export const keyRegex = () => new RegExp(KEY_SOURCE, "gi");

/** Безопасная проверка наличия ключей (без разделяемого lastIndex). */
export const hasKeys = (text: string) => new RegExp(KEY_SOURCE, "i").test(text);
...
export function unwrapHtmlSub(text: string): string {
  ...
  const best = [pre, body].find((t) => t && hasKeys(t));
...
export function regexFallback(text: string): string {
  const found = text.match(keyRegex());
