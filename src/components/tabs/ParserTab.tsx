...
export default function ParserTab({ onSend }: { onSend: (text: string) => void }) {
  ...
  const run = async () => {
    const targets = url
      ...
      .map((l) => l.trim())
      .filter(Boolean);
    if (!targets.length) return toast("Введите ссылку, крипт-ссылку или текст подписки", "err");
    setBusy(true);
    setLogs([]);
    setResult("");
    setFound(0);
    ...
    for (let i = 0; i < targets.length; i++) {
      ...
      if (targets.length > 1) log(`━━ источник ${i + 1}/${targets.length}: ${target.slice(0, 70)}`);
      try {
        ...
        if (res.content) chunks.push(res.content);
        total += res.count;
    ...
    // объединяем и убираем дубликаты строк
    const merged = [...new Set(chunks.join("\n").split(/\r?\n/).map((l) => l.trim()).filter(Boolean))];
    const content = merged.join("\n");
    const uniqueCount = parseInput(content).nodes.length;

    setResult(content);
    setFound(uniqueCount);
    if (targets.length > 1)
      log(`✓ объединено источников: ${targets.length}, уникальных ключей: ${uniqueCount} (из ${total})`);

    toast(
      uniqueCount ? `Спарсено ключей: ${uniqueCount}` : "Ответ получен, ключей нет",
      uniqueCount ? "ok" : "err"
    );
    setBusy(false);
  };
  ...
  return (
    <div className="grid gap-5 lg:grid-cols-2">
      ...
      >
        <Label>Ссылки на подписки · happ://crypt5/… · incy://crypt1/… (по одной на строку)</Label>
        <TextArea
          value={url}
          onChange={setUrl}
          rows={3}
          placeholder={"https://sub.example.com/abc\nhttps://panel2.example.com/sub/xyz\nhapp://crypt5/…"}
        />
        <p className="mt-1 text-[10.5px] text-slate-500">
          Несколько строк = пакетный режим: источники загружаются подряд, ключи объединяются и
          дедуплицируются.
        </p>
