...
import { PROTOCOL_META, type Protocol } from "../lib/types";
...
function ConverterCore({
  ...
  const processed = useMemo(() => {
    let list = parsed.nodes
      ...
      .map((n) => (renames[n.id] ? { ...n, name: renames[n.id] } : n));
  ...
  }, [parsed, excluded, renames, protoFilter, opts]);
  ...
  const lang: "yaml" | "json" | "text" =
    format === "clash"
      ? "yaml"
      : format === "base64" || format === "uri" || (format === "qwdtt" && qwdttMode !== "json")
        ? "text"
        : "json";
  ...
  return (
      <div className="mx-auto max-w-6xl">
        ...
        {/* ------------------------------ STATS ------------------------------ */}
        {processed.length > 0 && <Stats nodes={processed} />}
        ...
        {parsed.nodes.length > 0 && (
          <NodeGrid
            ...
            onRename={(id, name) => setRenames((r) => ({ ...r, [id]: name }))}
          />
        )}
